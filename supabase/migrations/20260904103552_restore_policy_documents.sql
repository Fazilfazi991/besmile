...
-- Recovered from supabase/migrations/20260814100000_batch14_policy_assistant.sql
...
revoke execute on function public.policy_audience_matches(uuid,uuid),public.policy_document_visible(uuid,boolean),public.create_policy_document_draft(jsonb,jsonb,jsonb),public.publish_policy_document(uuid),public.archive_policy_document(uuid),public.search_policy_sections(text,integer,text[]),public.check_policy_assistant_rate_limit() from public,anon;
grant execute on function public.policy_audience_matches(uuid,uuid),public.policy_document_visible(uuid,boolean) to authenticated;

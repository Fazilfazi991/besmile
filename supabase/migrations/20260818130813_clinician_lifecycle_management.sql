-- Soft clinician lifecycle with server-side authorization, operational safety,
-- and auditable state changes. Existing appointments, payables, and finance
-- rows keep their shared outsourced_doctors identifier.
...
begin
  if (new.archived_at is distinct from old.archived_at or new.status is distinct from old.status)
    and current_setting('app.clinician_lifecycle', true) is distinct from 'true'

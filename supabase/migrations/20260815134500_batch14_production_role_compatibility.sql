-- Forward-only production compatibility for the failed Batch 14 Policy Assistant migration.
...
begin
  if exists(select 1 from information_schema.columns where table_schema='public' and table_name='role_permissions' and column_name='role_id') then
    ...
    where (permission.code='policy_assistant.use' and role.code in ('staff','psychologist','intern','general_manager','director','chairman'))
       or (permission.code='policy_assistant.manage' and role.code in ('general_manager','director','chairman'))
    ...
    execute $seed$
      ...
      from (values ('Staff'),('Psychologist'),('Intern'),('General Manager'),('Director'),('Chairman')) seed(role_name)
    ...
    execute $seed$
      insert into public.role_permissions(role,permission_id)
      select role_name::public.employee_role,permission.id
      from (values ('General Manager'),('Director'),('Chairman')) seed(role_name)
...
create table if not exists public.policy_document_audiences (
  id uuid primary key default gen_random_uuid(),
...
create table if not exists public.policy_assistant_rate_limits (
...
create index if not exists policy_documents_current_idx on public.policy_documents(status,is_current,effective_date) where status='published';
create index if not exists policy_documents_type_idx on public.policy_documents(document_type,status);
create index if not exists policy_sections_document_idx on public.policy_sections(policy_document_id,chunk_order);
create index if not exists policy_sections_search_idx on public.policy_sections using gin(search_text);
create index if not exists policy_audiences_document_idx on public.policy_document_audiences(policy_document_id,audience_type,audience_value);
create unique index if not exists policy_one_current_version_idx on public.policy_documents(lower(title)) where status='published' and is_current;
...
drop policy if exists "policy documents scoped read" on public.policy_documents;
...
drop policy if exists "policy documents managers delete drafts" on public.policy_documents;
...
drop policy if exists "policy audiences scoped read" on public.policy_document_audiences;
...
drop policy if exists "policy sections scoped read" on public.policy_sections;
...
drop policy if exists "policy pdf manager upload" on storage.objects;
...
drop policy if exists "policy pdf scoped read" on storage.objects;
...
drop policy if exists "policy pdf manager cleanup" on storage.objects;

-- Harden the existing atomic invoice RPC without changing any applied migration.
create or replace function public.create_finance_invoice_atomic(
  target_invoice_number text, target_client uuid, target_customer_name text,
  target_customer_phone text, target_customer_email text, target_issue_date date,
  target_due_date date, target_discount numeric, target_tax numeric, target_notes text,
  target_status text, target_currency text, item_rows jsonb
)
...
begin
  ...
  if target_issue_date is null then
  ...
  if target_discount is null or target_tax is null or target_discount = 'NaN'::numeric or target_tax = 'NaN'::numeric or target_discount < 0 or target_tax < 0 then
    raise exception 'Tax and discount must be valid amounts that are zero or greater.' using errcode = '22023';
  end if;
  ...
  if exists (
    ...
    where nullif(btrim(item.description), '') is null
      or item.quantity is null or item.quantity = 'NaN'::numeric or item.quantity <= 0
      or item.rate is null or item.rate = 'NaN'::numeric or item.rate < 0
  ) then
    raise exception 'Each line item needs a description, positive quantity, and non-negative rate.' using errcode = '22023';
  end if;
  select sum(item.quantity * item.rate) - target_discount + target_tax into calculated_total
  from jsonb_to_recordset(item_rows) as item(description text, quantity numeric, rate numeric);
  if calculated_total is null or calculated_total = 'NaN'::numeric or calculated_total < 0 then
    raise exception 'Invoice total must be a valid amount that is zero or greater.' using errcode = '22023';
  end if;
  ...
  insert into public.finance_invoices(
    ...
    nullif(btrim(target_customer_email), ''), target_issue_date, target_due_date, target_discount,
    target_tax, nullif(btrim(target_notes), ''), target_status, coalesce(nullif(target_currency, ''), 'INR'), auth.uid()

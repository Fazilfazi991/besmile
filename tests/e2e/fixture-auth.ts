...
export function recordFixtureLoginRetry(role: string, reason = 'transient-transport') {
  mkdirSync('release-evidence', { recursive: true });
  appendFileSync('release-evidence/fixture-login-retries.jsonl', JSON.stringify({ action: 'fixture-login', role, attempt: 2, reason, time: new Date().toISOString() }) + '\n');
}

...
export const requiredProfileCompletionFields = [
  ...
  ['Work email','email'],
  ['Gender','gender'],
  ['Official ID','employee_code'],
  ['Department','department_id'],
  ['Designation','designation'],
  ['Role','role'],
] as const;
export function profileCompletion(profile:any){const fields=profile?.role==='chairman'||profile?.role==='director'?requiredProfileCompletionFields.filter(([,key])=>key!=='employee_code'):requiredProfileCompletionFields;const missing=fields.filter(([,key])=>{const value=profile?.[key];return !value||!String(value).trim()}).map(([label])=>label);return {percentage:Math.round(((fields.length-missing.length)/fields.length)*100),missing};}

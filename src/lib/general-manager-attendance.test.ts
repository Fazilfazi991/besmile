...
describe('General Manager attendance', () => {
  ...
  it('reuses the personal attendance page for GM while scoping every history query to the signed-in profile', () => {
    ...
    expect(employeeRepository).toMatch(/\.eq\(["']profile_id["'],\s*userId\)[\s\S]*?\.order\(["']work_date["']/);
    expect(employeeRepository).toMatch(/\.eq\(["']profile_id["'],\s*userId\)[\s\S]*?\.eq\(["']work_date["'],\s*workDate\)/);
  });
});

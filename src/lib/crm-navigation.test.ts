...
describe('CRM navigation', () => {
  it('shows the complete management CRM structure for a lead manager', () => {
    ...
    const crm = groups.find(group => group.title === 'CRM');
    expect(crm?.links.map(link => link.label)).toEqual(expect.arrayContaining(['CRM Dashboard', 'Leads Management', 'Follow-ups', 'Sales']));
  });

...
export function teamAttendanceState(attendance: TeamAttendanceRecord, onLeave = false) {
  ...
  if (!attendance) return { label: 'Punch-In Not Recorded', detail: undefined, tone: 'neutral' };
  if (attendance.clock_out) return { label: 'Punch-Out Recorded', detail: `Punch-Out ${formatTime(attendance.clock_out)}`, tone: 'clocked-out' };
  ...
  return { label: 'Present', detail: attendance.clock_in ? `Punch-In ${formatTime(attendance.clock_in)}` : undefined, tone: 'present' };
}

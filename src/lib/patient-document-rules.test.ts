...
describe('patient document safety', () => {
  ...
  it('accepts WebP patient documents with matching content type', () => { expect(validatePatientDocument({name:'scan.webp',type:'image/webp',size:128})).toBe('webp'); expect(() => validatePatientDocument({name:'scan.webp',type:'image/png',size:128})).toThrow(); });

...
export type EditableMeetingRecord = { id: string; organizer_id: string; title: string | null; agenda: string | null; start_at: string; end_at: string; meeting_type: string | null; venue: string | null; meeting_url: string | null; description: string | null; meeting_participants?: readonly MeetingParticipantRecord[] | null };
...
export function meetingFieldsFromRecord(meeting: EditableMeetingRecord): MeetingFields {
  ...
  return { title: meeting.title ?? '', agenda: meeting.agenda ?? '', date: start.date, start: start.time, end: end.time, type: meeting.meeting_type ?? 'office', venue: meeting.venue ?? '', url: meeting.meeting_url ?? '', notes: meeting.description ?? '', invitees: participantIdsFromMeeting(meeting.meeting_participants) };
}

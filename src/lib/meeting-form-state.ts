...
export function meetingFieldsFromRecord(meeting: EditableMeetingRecord): MeetingFields {
  ...
  return { host: meeting.host_user_id ?? '', title: meeting.title ?? '', agenda: meeting.agenda ?? '', date: start.date, start: start.time, end: end.time, type: meeting.meeting_type ?? 'office', venue: meeting.venue ?? '', url: meeting.meeting_url ?? '', notes: meeting.description ?? '', invitees: participantIdsFromMeeting(meeting.meeting_participants) };
}
...
export function canEditMeeting(permissions: Record<string, boolean>, meeting: Pick<EditableMeetingRecord, 'organizer_id' | 'host_user_id'>, currentProfileId: string | undefined) {
  return permissions['meetings.manage'] === true || !!currentProfileId && (meeting.organizer_id === currentProfileId || meeting.host_user_id === currentProfileId);
}

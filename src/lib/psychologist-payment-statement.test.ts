...
describe('psychologist payment statements', () => {
  ...
  it('supports outsourced clinicians without a BSmile Auth/profile record', () => {
    expect(migration).toContain('psychologist_profile_id uuid references public.profiles(id) on delete set null');
    expect(migration).not.toContain('psychologist_profile_id uuid not null');
    expect(migration).toContain('from public.outsourced_doctors doctor where doctor.id = selected_psychologist');
  });

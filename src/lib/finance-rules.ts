...
export const financeAccountBalance=(openingBalance:number|string|null|undefined,rows:{transaction_type:string;amount:number|string;archived_at?:string|null}[])=>Number(openingBalance||0)+rows.filter(x=>!x.archived_at).reduce((total,x)=>total+(['income','invoice_payment','adjustment'].includes(x.transaction_type)?Number(x.amount||0):-Number(x.amount||0)),0);

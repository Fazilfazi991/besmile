...
describe('Batch 9 business status',()=>{
 it('reuses canonical cash movement without specialized payment double counting',()=>{const totals=financeTotals([{transaction_type:'income',amount:10000},{transaction_type:'expense',amount:2000},{transaction_type:'payroll_payment',amount:3000},{transaction_type:'psychologist_payment',amount:1500}]);expect(totals).toEqual({income:10000,expenses:6500});expect(totals.income-totals.expenses).toBe(3500)});

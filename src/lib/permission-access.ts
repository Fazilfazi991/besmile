...
export function sectionNavigation(groups: readonly NavigationGroup[]): NavigationSection[] {
  const sections: NavigationSection[] = [
    { title: 'Overview', links: [] }, { title: 'Operations', links: [] },
    { title: 'Performance', links: [] }, { title: 'Communication', links: [] }, { title: 'CRM', links: [] },
    { title: 'Finance', links: [] }, { title: 'Data & Settings', links: [] },
  ];
  const sectionFor = (link: NavigationLink) => {
    ...
    if (/^(Chat|Teams|Announcements|Notifications|Customer Feedback)$/.test(label)) return 'Communication';
    if (href.includes('/tasks') || href.includes('/attendance') || href.includes('/leave') || href.includes('/calendar') || href.includes('/holidays') || href.includes('/meetings') || href.includes('/ideas') || href.includes('/doctor-scheduling')) return 'Performance';
...
export function activeNavigationHref(pathname: string, groups: readonly NavigationGroup[]) {
  ...
  for (const link of groups.flatMap((group) => group.links)) for (const candidate of [link.href, ...(link.activeHrefs || [])]) {
...
export const employeeNavigation: readonly NavigationGroup[] = [
  ...
  { title: 'COMMUNICATION', links: [{ label: 'Chat', href: '/employee/chat', requirement: anyOf('chat.use') }, { label: 'Notifications', href: '/employee/notifications' }, { label: 'Profile', href: '/employee/profile' }] },

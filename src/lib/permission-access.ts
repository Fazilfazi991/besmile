...
export function workspaceLandingPath(role?: string | null) {
  return normalizeRole(role) === "super_admin" || isManagementRole(role)
    ? "/admin"
    : "/employee/dashboard";
}
...
const anyOf = (...permissions: string[]): PermissionRequirement => ({
  anyOf: permissions,
});
const requireAllAndAny = (
  all: readonly string[],
  any: readonly string[],
): PermissionRequirement => ({ allOf: all, anyOf: any });
const requireAnyWithout = (
  any: readonly string[],
  none: readonly string[],
): PermissionRequirement => ({ anyOf: any, noneOf: none });
...
export function permissionAllows(
  ...
  const allAllowed =
    !requirement.allOf ||
    requirement.allOf.every((permission) => granted.has(permission));
  const anyAllowed =
    !requirement.anyOf ||
    requirement.anyOf.some((permission) => granted.has(permission));
  const noneBlocked =
    !requirement.noneOf ||
    requirement.noneOf.every((permission) => !granted.has(permission));
  ...
  return groups
    .map((group) => ({
      ...
      links: group.links.filter((link) =>
        permissionAllows(granted, link.requirement),
      ),
    }))
    .filter((group) => group.links.length > 0);
}

/** Presents the permission-filtered canonical routes as compact sidebar sections. */
export function sectionNavigation(
  ...
  const sectionFor = (link: NavigationLink) => {
    ...
    if (
      ...
      href.includes("/meetings") ||
      href.includes("/ideas") ||
      href.includes("/doctor-scheduling")
    )
      return "Performance";
    return "Data & Settings";
  };
  for (const link of groups.flatMap((group) => group.links))
    sections
      .find((section) => section.title === sectionFor(link))!
      .links.push(link);
...
export function adminRouteRequirement(path: string): PermissionRequirement {
  ...
  if (path.startsWith("/admin/calendar"))
    return anyOf("meetings.view", "meetings.create", "meetings.manage");
  ...
  if (path.startsWith("/admin/leaves"))
    return anyOf("leave.approve", "leave.manage", "leave.review");
  if (path === "/admin/tasks") return anyOf("tasks.manage", "tasks.assign");
  if (path === "/admin/task-access") return anyOf("tasks.manage_access");
  if (path.startsWith("/admin/ideas/categories"))
    return anyOf("ideas.manage_categories");
  ...
  if (path.startsWith("/admin/announcements"))
    return anyOf("announcements.manage");
  if (path.startsWith("/admin/notifications"))
    return anyOf("notifications.view");
  if (path === "/admin/finance/invoices/new") return anyOf("invoices.manage");
  if (path.startsWith("/admin/finance/invoices"))
    return anyOf("invoices.view", "invoices.manage");
  if (path === "/admin/finance/payroll/settings")
    return anyOf("payroll.manage");
  if (path.startsWith("/admin/finance/psychologist-payments"))
    return anyOf("psychologist_payments.view");
  if (path.startsWith("/admin/finance/payroll"))
    return anyOf("payroll.view", "payroll.manage");
  if (path.startsWith("/admin/finance/reports"))
    return anyOf("reports.finance.view", "reports.view");
  if (path.startsWith("/admin/reports"))
    return anyOf("reports.finance.view", "reports.view");
  if (path.startsWith("/admin/finance/income"))
    return anyOf("income.view", "income.manage");
  if (path.startsWith("/admin/finance/expenses"))
    return anyOf("expenses.view", "expenses.manage");
  if (path.startsWith("/admin/finance"))
    return anyOf("finance.dashboard.view", "finance.view");
...
export function employeeRouteRequirement(
  ...
  if (
    ...
    return undefined;
  if (path.startsWith("/employee/announcements"))
    return anyOf("announcements.view", "announcements.manage");
  ...
  if (path.startsWith("/employee/meetings"))
    return anyOf("meetings.view", "meetings.create", "meetings.manage");
  if (path.startsWith("/employee/leaves"))
    return anyOf(
      "leave.self",
      "leave.request",
      "leave.view",
  ...
  if (path.startsWith("/employee/tasks/access"))
    return anyOf("tasks.manage_access");
    ...
    links: [
      {
        label: "Dashboard",
        href: "/admin",
        requirement: anyOf("admin.access", "dashboard.view"),
      },
      {
        label: "Reports",
        href: "/admin/reports",
        requirement: anyOf("reports.finance.view", "reports.view"),
      },
    ],
  },
  {
    ...
    links: [
      ...
      },
      {
        ...
        requirement: anyOf("documents.manage", "documents.employee.manage"),
      },
      {
        ...
        requirement: anyOf(
          ...
          "documents.administration.manage",
        ),
      },
    ],
  },
    ...
    links: [
      {
        ...
        requirement: anyOf("attendance.self"),
      },
      {
      ...
      },
      {
        ...
        href: "/admin/holidays",
        requirement: anyOf("holiday_calendar.manage"),
      },
      {
        label: "Meetings",
        href: "/admin/meetings",
      ...
      },
      {
        ...
        href: "/admin/leaves",
        requirement: anyOf("leave.approve", "leave.manage", "leave.review"),
      },
      {
        label: "Tasks",
        href: "/admin/tasks",
        requirement: anyOf("tasks.manage", "tasks.assign"),
      },
      {
        label: "Task Access",
        href: "/admin/task-access",
        requirement: anyOf("tasks.manage_access"),
      },
      {
        label: "Appointment & Scheduling",
        href: "/admin/doctor-scheduling",
        requirement: anyOf("doctor_scheduling.view"),
      },
      {
      ...
      },
      {
        ...
        requirement: anyOf("customer_feedback.view"),
      },
      {
        label: "Innovation Categories",
        href: "/admin/ideas/categories",
        requirement: anyOf("ideas.manage_categories"),
      },
    ],
  },
  {
    ...
    links: [
      { label: "Chat", href: "/admin/chat", requirement: anyOf("chat.use") },
      {
        ...
        requirement: anyOf("announcements.manage"),
      },
      {
        label: "Notifications",
        href: "/admin/notifications",
        requirement: anyOf("notifications.view"),
      },
      {
        label: "Profile",
        href: "/admin/profile",
  ...
  },
  {
    ...
    links: [
      ...
      },
      {
        ...
        requirement: anyOf("crm.import"),
      },
      {
        label: "Sales",
        href: "/admin/crm/sales",
        requirement: anyOf("crm.manage_all", "crm.view_team", "sales.view"),
      },
    ],
  },
  {
    ...
    links: [
      {
        ...
        href: "/admin/finance",
        requirement: anyOf("finance.dashboard.view", "finance.view"),
      },
      {
        label: "Income",
        href: "/admin/finance/income",
        requirement: anyOf("income.view", "income.manage"),
      },
      {
        label: "Expenses",
        href: "/admin/finance/expenses",
        requirement: anyOf("expenses.view", "expenses.manage"),
      },
      {
        label: "Invoices",
        href: "/admin/finance/invoices",
        requirement: anyOf("invoices.view", "invoices.manage"),
      },
      {
        label: "Payroll",
        href: "/admin/finance/payroll",
        requirement: anyOf("payroll.view", "payroll.manage"),
      },
      {
        label: "Psychologist Payments",
        href: "/admin/finance/psychologist-payments",
        requirement: anyOf("psychologist_payments.view"),
      },
      {
        label: "Reports",
    ...
    links: [
      {
        label: "Roles & Access",
        href: "/admin/access",
        requirement: anyOf("roles.manage", "permissions.manage"),
      },
    ],
  },
];
    ...
    links: [
      {
        label: "Dashboard",
        href: "/employee/dashboard",
        requirement: anyOf("dashboard.view"),
      },
      {
        label: "My Attendance",
      ...
      },
      {
        label: "Tasks",
        href: "/employee/tasks",
        requirement: anyOf("tasks.view_self", "tasks.assign"),
      },
      {
        label: "Manage Tasks",
      ...
      },
      {
        ...
        activeHrefs: ["/admin/doctor-scheduling"],
        requirement: anyOf("doctor_scheduling.view"),
      },
      {
        label: "Innovation Hub",
        href: "/employee/ideas",
        activeHrefs: ["/admin/ideas"],
        requirement: anyOf("ideas.view"),
      },
      {
        label: "Clients",
      ...
      },
      {
        ...
        href: "/employee/assigned-patients",
        activeHrefs: ["/employee/patients"],
  ...
  },
  {
    ...
    links: [
      ...
      },
      {
        ...
        requirement: requireAllAndAny(
          ["admin.shell"],
          ["documents.employee.manage", "documents.administration.manage"],
        ),
      },
    ],
  },
  ...
  },
  {
    ...
    links: [
      ...
      },
      {
        ...
        requirement: requireAllAndAny(
          ["admin.shell"],
          ["crm.manage_all", "crm.view_team", "leads.view"],
        ),
      },
      {
        label: "Follow-ups",
      ...
      },
      {
        ...
        requirement: requireAllAndAny(
          ["admin.shell"],
          ["crm.manage_all", "crm.view_team", "sales.view"],
        ),
      },
      {
        label: "CRM Dashboard",
        href: "/employee/crm",
        requirement: requireAnyWithout(
          [
            "crm.view_assigned",
            "crm.view_team",
            "crm.manage_all",
      ...
      {
        label: "My Sales",
        href: "/employee/crm/sales",
        requirement: requireAnyWithout(
          ["crm.view_team", "crm.manage_all", "sales.view"],
          ["admin.shell"],
        ),
      },
    ],
...
export const navigationPermissionCodes = [
  ...new Set(
    [...adminNavigation, ...employeeNavigation].flatMap((group) =>
      group.links.flatMap((link) => [
        ...(link.requirement?.anyOf || []),
        ...(link.requirement?.allOf || []),
        ...(link.requirement?.noneOf || []),
      ]),
    ),
  ),
];

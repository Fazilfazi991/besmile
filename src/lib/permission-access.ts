...
export function activeNavigationHref(pathname: string, groups: readonly NavigationGroup[]) {
  ...
  for (const link of groups.flatMap((group) => group.links)) {
    for (const candidate of [link.href, ...(link.activeHrefs || [])]) {
      const matches = pathname === candidate || (!link.exact && candidate !== '/admin' && pathname.startsWith(`${candidate}/`));
      if (!matches) continue;
      const score = candidate.length + (pathname === candidate ? 10_000 : 0);
...
export function adminRouteRequirement(path: string): PermissionRequirement {
  if (path.startsWith('/admin/daily-work')) return anyOf('attendance.view', 'attendance.manage');
  ...
  if (path.startsWith('/admin/holidays')) return anyOf('holiday_calendar.manage');
  ...
  if (path.startsWith('/admin/my-leave')) return anyOf('leave.self', 'leave.request', 'leave.view', 'leave.manage', 'leave.approve', 'leave.review');
...
export function employeeRouteRequirement(path: string): PermissionRequirement | undefined {
  if (path.startsWith('/employee/daily-work')) return anyOf('attendance.self', 'attendance.view_self', 'attendance.view', 'attendance.manage');
  ...
  if (path.startsWith('/employee/chat')) return undefined;
  ...
  if (path.startsWith('/employee/holidays')) return undefined;
  if (path.startsWith('/employee/calendar')) return undefined;
  if (path.startsWith('/employee/meetings')) return undefined;
...
export const adminNavigation: readonly NavigationGroup[] = [
  { title: 'DASHBOARD', links: [{ label: 'Dashboard', href: '/admin', requirement: anyOf('admin.access', 'dashboard.view') }] },
  { title: 'HR & EMPLOYEE MANAGEMENT', links: [{ label: 'Employees', href: '/admin/employees', requirement: anyOf('employees.view') }, { label: 'My Attendance', href: '/admin/my-attendance', requirement: anyOf('attendance.self') }, { label: 'Staff Attendance', href: '/admin/attendance', requirement: anyOf('attendance.view', 'attendance.manage') }, { label: 'My Leave', href: '/admin/my-leave', requirement: anyOf('leave.self', 'leave.request', 'leave.view', 'leave.manage', 'leave.approve', 'leave.review') }, { label: 'Leave Approvals', href: '/admin/leaves', requirement: anyOf('leave.approve', 'leave.manage', 'leave.review') }, { label: 'My Profile', href: '/admin/profile', requirement: anyOf('admin.access', 'dashboard.view') }] },
  { title: 'WORK MANAGEMENT', links: [{ label: 'Tasks', href: '/admin/tasks', requirement: anyOf('tasks.assign') }, { label: 'Daily Work Updates', href: '/admin/daily-work', requirement: anyOf('attendance.view', 'attendance.manage') }, { label: 'Task Access', href: '/admin/task-access', requirement: anyOf('tasks.manage_access') }, { label: 'My Calendar', href: '/admin/calendar', requirement: anyOf('meetings.view', 'meetings.create', 'meetings.manage') }, { label: 'Holiday Calendar', href: '/admin/holidays', requirement: anyOf('holiday_calendar.manage') }, { label: 'Meetings', href: '/admin/meetings', requirement: anyOf('meetings.view', 'meetings.create', 'meetings.manage') }, { label: 'Innovation Hub', href: '/admin/ideas', requirement: anyOf('ideas.view', 'ideas.manage_status', 'ideas.view_reports') }, { label: 'Innovation Categories', href: '/admin/ideas/categories', requirement: anyOf('ideas.manage_categories') }] },
  { title: 'CLIENT & CRM', links: [{ label: 'CRM Overview', href: '/admin/crm', requirement: anyOf('crm.manage_all', 'crm.view_team', 'leads.view') }, { label: 'Leads', href: '/admin/crm/leads', requirement: anyOf('crm.manage_all', 'crm.view_team', 'leads.view') }, { label: 'Follow-ups', href: '/admin/crm/follow-ups', requirement: anyOf('crm.manage_all', 'crm.view_team', 'leads.view') }, { label: 'Import Leads', href: '/admin/crm/import', requirement: anyOf('crm.import') }, { label: 'Sales', href: '/admin/crm/sales', requirement: anyOf('crm.manage_all', 'crm.view_team', 'sales.view') }, { label: 'Clients', href: '/admin/patients', requirement: anyOf('patients.view', 'patients.view_assigned', 'patients.view_all') }, { label: 'Appointment & Scheduling', href: '/admin/doctor-scheduling', requirement: anyOf('doctor_scheduling.view') }, { label: 'Customer Feedback', href: '/admin/customer-feedback', requirement: anyOf('customer_feedback.view') }] },
  { title: 'FINANCE & ACCOUNTS', links: [{ label: 'Finance Dashboard', href: '/admin/finance', requirement: anyOf('finance.dashboard.view', 'finance.view') }, { label: 'Income', href: '/admin/finance/income', requirement: anyOf('income.view', 'income.manage') }, { label: 'Expenses', href: '/admin/finance/expenses', requirement: anyOf('expenses.view', 'expenses.manage') }, { label: 'Invoices', href: '/admin/finance/invoices', requirement: anyOf('invoices.view', 'invoices.manage') }, { label: 'Payroll', href: '/admin/finance/payroll', requirement: anyOf('payroll.view', 'payroll.manage') }, { label: 'Psychologist Payments', href: '/admin/finance/psychologist-payments', requirement: anyOf('psychologist_payments.view') }, { label: 'Finance Reports', href: '/admin/finance/reports', requirement: anyOf('reports.finance.view', 'reports.view') }] },
  { title: 'DOCUMENTS & REPORTS', links: [{ label: 'Official Documents', href: '/admin/documents/generate', requirement: anyOf('documents.manage', 'documents.employee.manage') }, { label: 'Operational Documents', href: '/admin/documents', exact: true, requirement: anyOf('documents.manage', 'documents.employee.manage', 'documents.administration.manage') }, { label: 'Operational Reports', href: '/admin/reports', requirement: anyOf('reports.finance.view', 'reports.view') }] },
  ...
  { title: 'ADMINISTRATION / SYSTEM', links: [{ label: 'Roles & Access', href: '/admin/access', requirement: anyOf('roles.manage', 'permissions.manage') }] },
];

export const employeeNavigation: readonly NavigationGroup[] = [
  { title: 'HOME', links: [{ label: 'Dashboard', href: '/employee/dashboard', requirement: anyOf('dashboard.view') }] },
  { title: 'MY WORK', links: [{ label: 'My Tasks', href: '/employee/tasks', requirement: anyOf('tasks.view_self', 'tasks.assign') }, { label: 'Daily Work Update', href: '/employee/daily-work', requirement: anyOf('attendance.self', 'attendance.view_self', 'attendance.view', 'attendance.manage') }, { label: 'Manage Tasks', href: '/employee/tasks/manage', requirement: anyOf('tasks.assign') }, { label: 'My Calendar', href: '/employee/calendar' }, { label: 'Holiday Calendar', href: '/employee/holidays' }, { label: 'Meetings', href: '/employee/meetings' }, { label: 'Innovation Hub', href: '/employee/ideas', activeHrefs: ['/admin/ideas'], requirement: anyOf('ideas.view') }] },
  { title: 'ATTENDANCE & LEAVE', links: [{ label: 'My Attendance', href: '/employee/attendance', requirement: anyOf('attendance.self', 'attendance.view_self', 'attendance.view', 'attendance.manage') }, { label: 'Leave Requests', href: '/employee/leaves', requirement: anyOf('leave.self', 'leave.request', 'leave.view', 'leave.manage', 'leave.approve') }] },
  { title: 'HR & EMPLOYEE MANAGEMENT', links: [{ label: 'Employees', href: '/admin/employees', requirement: requireAllAndAny(['admin.shell'], ['employees.view']) }] },

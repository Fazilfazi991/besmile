...
const timezoneMigration = readFileSync(resolve(process.cwd(), 'supabase/migrations/20260808181000_business_timezone_ist.sql'), 'utf8');

describe('document expiry migration', () => {
  it('uses a business-timezone scheduler with idempotent per-window deliveries', () => {
    expect(timezoneMigration).toContain("'Asia/Kolkata'");
    expect(timezoneMigration).toContain('public.business_today()');

...
export function buildExecutiveFinanceView(transactions: Transaction[], period: ExecutivePeriod, timeZone: string, now = new Date()) {
  ...
  const daily = period === 'month' || period === 'previous_month';
  const bucketLength = daily ? 7 : period === 'quarter' ? 14 : 31;
  const bucketCount = Math.ceil((Math.round((end.getTime() - start.getTime()) / 86400000) + 1) / bucketLength);
  const trend = Array.from({ length: bucketCount }, (_, index) => {
    const bucketStart = new Date(start.getTime() + index * bucketLength * 86400000);
    const bucketEnd = new Date(Math.min(end.getTime(), bucketStart.getTime() + (bucketLength - 1) * 86400000));

...
export default function ClinicianProfilePage() {
  ...
  return <section className="space-y-5"><div><h1 className="text-2xl font-bold">Clinician Profile</h1><p className="text-slate-600">Account and contact settings. This is not an employee or payroll record.</p></div>{notice && <p className="rounded bg-emerald-50 p-3 text-emerald-800">{notice}</p>}{error && <p className="rounded bg-rose-50 p-3 text-rose-800">{error}</p>}<form className="card grid gap-4 p-5 md:grid-cols-2" onSubmit={save}><label className="text-sm font-medium">Full name<input className="input mt-1" required value={form.full_name} onChange={event => setForm({ ...form, full_name: event.target.value })} /></label><label className="text-sm font-medium">Phone<input className="input mt-1" value={form.phone} onChange={event => setForm({ ...form, phone: event.target.value })} /></label><label className="text-sm font-medium">Personal email<input className="input mt-1" type="email" value={form.personal_email} onChange={event => setForm({ ...form, personal_email: event.target.value })} /></label><div className="md:col-span-2"><button className="btn btn-primary">Save profile</button></div></form><BrowserPushSettings /></section>;
}

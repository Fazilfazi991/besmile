...
export default function ExecutiveDashboard() {
  ...
  const kpis = [
    { icon: 'Employees', tone: 'people', title: 'Total employees', value: summary.employees, detail: `${summary.presentToday || 0} active today`, href: '/admin/employees' },
    { icon: 'Attendance', tone: 'attendance', title: 'Present today', value: summary.presentToday, detail: `${presentPercent}% attendance`, href: '/admin/attendance' },
    { icon: 'Leads', tone: 'crm', title: 'Active leads', value: summary.leads, detail: `${summary.newLeads || 0} new today`, href: '/admin/crm' },
    ...
    ...(canViewFinance ? [
      { icon: 'Finance Dashboard', tone: 'finance', title: 'Monthly revenue', value: inr(monthly.income), detail: trendLabel(monthly.income, monthly.previousIncome), href: '/admin/finance' },
  ...
  return <section className="executive-dashboard">
    ...
    {canViewTeam && <TeamAttendanceStrip employees={team} loading={!team} canOpenEmployees={canOpenEmployees} standardHub attendanceAvailable={teamAvailability.attendance} leaveAvailable={teamAvailability.leave} />}

    <div className="executive-kpis">{kpis.map(kpi => <Link href={kpi.href} className={`executive-kpi executive-kpi-${kpi.tone}`} key={kpi.title}><ModuleIcon label={kpi.icon} className="executive-kpi-icon" /><div><p>{kpi.title}</p><b>{kpi.value}</b><small>{kpi.detail}</small></div><span className="executive-arrow">→</span></Link>)}</div>
    <section className="standard-needs-attention"><h2>Needs attention</h2><p>Overdue and upcoming — task deadlines, pending client follow-ups, and paid-session conversions due, sorted by urgency.</p><div>{summary.overdueTasks || summary.followupsDue ? `${(summary.overdueTasks || 0) + (summary.followupsDue || 0)} item${(summary.overdueTasks || 0) + (summary.followupsDue || 0) === 1 ? '' : 's'} need review.` : 'Nothing overdue or due soon.'}</div></section>

...
function OperationalDashboard() {
  ...
  const attendanceAction = async (action: 'clockIn' | 'clockOut' | 'startBreak' | 'endBreak') => { if (!profile || attendanceRequest.current) return; attendanceRequest.current = true; setRefreshingAttendance(true); setNotice(''); setAttendanceError(''); try { if (action === 'clockIn' || action === 'clockOut') setAttendanceStatus(locationCheckingMessage); if (action === 'clockIn') await employeeRepository.clockIn(profile.id, await freshLocation('Clock In')); if (action === 'clockOut' && todayAttendance) await employeeRepository.clockOut(todayAttendance.id, await freshLocation('Clock Out')); if (action === 'startBreak' && todayAttendance) await employeeRepository.startBreak(todayAttendance.id); if (action === 'endBreak' && activeBreak) await employeeRepository.endBreak(activeBreak.id); const confirmed = await employeeRepository.attendanceToday(profile.id); const confirmedBreak = confirmed.attendance?.attendance_breaks?.some((item: any) => !item.ended_at); if (!confirmed.attendance?.clock_in || (action === 'clockOut' && !confirmed.attendance.clock_out) || (action === 'startBreak' && !confirmedBreak) || (action === 'endBreak' && confirmedBreak)) throw new Error('Attendance update could not be confirmed. Please try again.'); setTodayAttendance(confirmed.attendance); await load(); setNotice(action === 'clockIn' ? 'Clocked in.' : action === 'clockOut' ? 'Clocked out.' : action === 'startBreak' ? 'Break started.' : 'Break ended.'); } catch (caughtError: any) { console.error('GM attendance action failed', caughtError); setAttendanceError(caughtError.message || 'Attendance could not be updated.'); } finally { attendanceRequest.current = false; setAttendanceStatus(''); setRefreshingAttendance(false); } };

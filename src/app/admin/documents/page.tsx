...
export default function AdminDocumentsPage() {
  ...
  const [share, setShare] = useState({ title: '', description: '', category: 'Policy', profileIds: [] as string[] });
  ...
  const upload = async (event: FormEvent) => {
    ...
    try {
      ...
      setFile(null);
      setShare({ title: '', description: '', category: 'Policy', profileIds: [] });
      await load();
    } catch (caught: any) {
      setError(caught.message);
    }
  ...
  return <section className="space-y-6">
    ...
    <div className="card overflow-hidden"><h2 className="border-b p-5 font-bold">Company documents</h2>{documents.map(document => <div className="flex justify-between border-b p-4" key={document.id}><span><b>{document.title}</b><small className="ml-2 text-slate-500">{document.category}</small><p className="text-sm text-slate-600">{document.description}</p></span><button className="text-sm underline" onClick={() => void open(document.storage_path)}>Preview / download</button></div>)}</div>

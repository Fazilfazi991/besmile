...
export default function InvoiceDetail() {
  ...
  return <section className="mx-auto max-w-5xl space-y-5 print:max-w-none">
    ...
    {paymentOpen && <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/40 p-4"><form className="card w-full max-w-lg p-6" onSubmit={recordPayment}><div className="flex items-start justify-between gap-3"><div><h2 className="text-lg font-bold">Record payment</h2><p className="mt-1 text-sm text-slate-600">Outstanding balance: <b>{inr(totals.outstanding)}</b></p></div><button className="text-slate-500 hover:text-slate-950" type="button" onClick={() => setPaymentOpen(false)}>Close</button></div><div className="mt-5 grid gap-4 sm:grid-cols-2"><Label label="Payment amount"><input required className="input" min="0.01" max={totals.outstanding} step="0.01" type="number" value={payment.amount} onChange={event => setPayment({ ...payment, amount: event.target.value })} /></Label><Label label="Payment date"><input required className="input" type="date" value={payment.payment_date} onChange={event => setPayment({ ...payment, payment_date: event.target.value })} /></Label><Label label="Receiving account"><select required className="input" value={payment.account_id} onChange={event => setPayment({ ...payment, account_id: event.target.value })}><option value="">Select account</option>{accounts.map(account => <option value={account.id} key={account.id}>{account.name}</option>)}</select></Label><Label label="Payment method"><select className="input" value={payment.payment_method} onChange={event => setPayment({ ...payment, payment_method: event.target.value })}>{['cash', 'bank_transfer', 'upi', 'card'].map(value => <option value={value} key={value}>{value.replaceAll('_', ' ')}</option>)}</select></Label><Label label="Reference"><input className="input" value={payment.reference} onChange={event => setPayment({ ...payment, reference: event.target.value })} /></Label><Label label="Notes"><input className="input" value={payment.notes} onChange={event => setPayment({ ...payment, notes: event.target.value })} /></Label></div><div className="mt-6 flex justify-end gap-2"><button className="btn border" type="button" onClick={() => setPaymentOpen(false)}>Cancel</button><button className="btn btn-primary" disabled={saving}>{saving ? 'Recording…' : 'Record payment'}</button></div></form></div>}
  </section>;
}

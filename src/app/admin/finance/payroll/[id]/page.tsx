...
function PayrollActions({ entry, canManage, run, pdfBusy, accounts, setAdjustment, setPayment, download }: any) {
  return <div className="flex flex-wrap gap-3">{canManage && run.status === 'draft' && <button className="text-teal-700 underline" onClick={() => setAdjustment({ ...entry, adjustment_type: 'bonus', amount: '', reason: '' })}>Add adjustment</button>}{canManage && run.status === 'approved' && entry.payment_status === 'approved' && <button disabled={!accounts.length} className="text-teal-700 underline" onClick={() => setPayment({ ...entry, account_id: accounts[0]?.id || '', payment_date: today(), payment_method: 'bank_transfer', payment_reference: '' })}>Mark paid</button>}{entry.payment_status !== 'draft' && <button className="text-teal-700 underline" disabled={pdfBusy} onClick={() => void download(`/api/payroll/entries/${entry.id}/payslip`)}>Payslip</button>}</div>;
}

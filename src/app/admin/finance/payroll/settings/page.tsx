...
export default function SalarySettings() {
  ...
  const load = async () => {
    try {
      ...
      setSettings(salarySettings); setStaff(employees.data.filter((employee: any) => employee.status === 'active'));
  ...
  return <section className="space-y-4">
    <div className="flex flex-wrap justify-between gap-3"><div><h1 className="text-2xl font-bold">Salary settings</h1><p className="text-sm text-slate-500">Active staff only are eligible for new payroll runs.</p></div><button disabled={loading || !staff.length} className="btn btn-primary" onClick={() => setEdit({ profile_id: staff[0]?.id || '', basic_salary: 0, default_allowances: 0, default_deductions: 0, effective_date: today(), is_active: true })}>Add setting</button></div>

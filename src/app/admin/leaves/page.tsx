...
export default function LeaveApprovalsPage() {
  ...
  const load = async () => {
    try {
      ...
      setError('');
    } catch (cause: any) { setError(cause.message || 'Leave requests could not be loaded.'); }
  };
  ...
  const review = async (id: string, status: 'approved' | 'rejected') => {
    if (!profile) return;
    setBusy(id); setError('');
    try { await adminRepository.reviewLeaveRequest(id, status, comments[id] || '', profile.id); await load(); }
    catch (cause: any) { setError(cause.message || 'Leave review could not be saved.'); }

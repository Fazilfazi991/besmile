...
import { adminNavigation, filterNavigation, isManagementRole, navigationPermissionCodes } from '@/lib/permission-access';
...
export default async function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return <div className="app-shell employee-shell">
    ...
    <main className="app-main">
      <header className="app-topbar"><div><p className="eyebrow">BSMILE CONTROL CENTER</p><h1>{profile.role === 'super_admin' ? 'Super Admin Workspace' : 'Management Workspace'}</h1></div><div className="flex items-center gap-3"><GlobalCommandCenter mode="admin" userId={user.id} canEmployees={allowed.has('employees.view')} canCrm={allowed.has('crm.manage_all') || allowed.has('crm.view_team') || allowed.has('leads.view')} canInvoices={allowed.has('invoices.view') || allowed.has('invoices.manage')} /><a className="topbar-user" href="/admin/access"><span>{name.slice(0, 1).toUpperCase()}</span><div><b>{name}</b><small>{profile.role === 'super_admin' ? 'Super Admin' : profile.designation || 'Management'}</small></div></a></div></header>

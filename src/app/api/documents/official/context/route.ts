...
export async function GET() {
  ...
  const [profile, employees, history] = await Promise.all([
    ...
    db.from('profiles').select('id,full_name,designation,joining_date,department:departments(name)').eq('is_employee', true).eq('workforce_visible', true).in('status', ['active', 'intern', 'probation']).order('full_name').limit(250),

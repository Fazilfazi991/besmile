...
export async function POST(request: Request, { params }: { params: Promise<{ patientId: string }> }) {
  ...
  } catch (error) { return NextResponse.json({ error: normalizeClientError(error, 'Unable to upload this document.') }, { status: 400 }); }
}

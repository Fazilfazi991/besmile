...
export default function EmployeeDashboard() {
  ...
  const attendanceAction = async (action: 'clockIn' | 'clockOut' | 'startBreak' | 'endBreak') => {
    ...
    try {
      ...
      if (action === 'clockOut' && todayAttendance) await employeeRepository.clockOut(todayAttendance.id, await freshLocation('Punch-Out'));
  ...
  return <div className="employee-dashboard">
    ...
    {canAttendance && <section className="attendance-card">
      <div><p className="eyebrow">TODAY&apos;S ATTENDANCE</p><h3>{!todayAttendance ? 'Punch-In not recorded' : todayAttendance.clock_out ? 'Day completed' : activeBreak ? 'On a break' : 'Working today'}</h3><p>Punch-In: <b>{fmtTime(todayAttendance?.clock_in)}</b> &nbsp; Punch-Out: <b>{fmtTime(todayAttendance?.clock_out)}</b></p><p className="attendance-duration">{workingDuration(todayAttendance, activeBreak)}</p></div>
      <div className="attendance-actions">
        {!todayAttendance && <button className="button button-primary" disabled={refreshing} onClick={() => void attendanceAction('clockIn')}>{attendanceStatus ? 'Checking location...' : error === locationBlockedMessage ? 'Try again' : 'Punch-In'}</button>}

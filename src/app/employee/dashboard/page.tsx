...
export default function EmployeeDashboard() {
  ...
  const load = async (quiet = false) => {
    ...
    try {
      ...
      const results = await Promise.allSettled([
        employeeRepository.attendanceHistory(signedInProfile.id), employeeRepository.myTasks(signedInProfile.id),
        employeeRepository.leaveHistory(signedInProfile.id), employeeRepository.documentRequests(signedInProfile.id),
        employeeRepository.announcements(signedInProfile.id), employeeRepository.notifications(signedInProfile.id, 0, 20),
        employeeRepository.profile(signedInProfile.id), employeeRepository.myCrmLeads(signedInProfile.id),
        employeeRepository.myCrmFollowups(signedInProfile.id), employeeRepository.myCrmSales(signedInProfile.id),
      ]);
      const [attendance, tasks, leaves, documents, announcements, notifications, richProfile, crmLeads, crmFollowups, crmSales] = results;
      setData({
        attendance: attendance.status === 'fulfilled' ? attendance.value : [], tasks: tasks.status === 'fulfilled' ? tasks.value : [],
        leaves: leaves.status === 'fulfilled' ? leaves.value : [], documents: documents.status === 'fulfilled' ? documents.value : [],
        announcements: announcements.status === 'fulfilled' ? announcements.value : [], notifications: notifications.status === 'fulfilled' ? notifications.value : [],
        richProfile: richProfile.status === 'fulfilled' ? richProfile.value : signedInProfile,
        crmLeads: crmLeads.status === 'fulfilled' ? crmLeads.value : [], crmFollowups: crmFollowups.status === 'fulfilled' ? crmFollowups.value : [], crmSales: crmSales.status === 'fulfilled' ? crmSales.value : [],
      });
      const failed = results.filter(result => result.status === 'rejected').length;
      setError(failed ? `${failed} dashboard section${failed === 1 ? '' : 's'} could not be refreshed.` : '');
    } catch (caughtError: any) { setError(caughtError.message || 'Unable to load your workspace.'); }
  ...
  return <div className="employee-dashboard">
    ...
    <section className="dashboard-metrics">
      <Metric label="Pending tasks" value={activeTasks.length} href="/employee/tasks" />
      <Metric label="Due today" value={activeTasks.filter((item: any) => item.tasks?.due_date === today).length} href="/employee/tasks" />
      <Metric label="Pending leave" value={pendingLeaves.length} href="/employee/leaves" />
      <Metric label="Unread updates" value={unreadNotifications} href="/employee/notifications" />
      <Metric label="Unread announcements" value={unreadAnnouncements} href="/employee/announcements" />
      <Metric label="Document requests" value={pendingDocuments.length} href="/employee/documents" />
      <Metric label="My leads" value={crmLeads.length} href="/employee/crm/leads" />
      <Metric label="CRM follow-ups today" value={crmFollowups.filter((item: any) => String(item.next_follow_up_at || '').slice(0, 10) === today).length} href="/employee/crm/follow-ups" />
      <Metric label="Hot leads" value={crmLeads.filter((item: any) => item.temperature === 'hot').length} href="/employee/crm/leads" />
      <Metric label="My sales" value={crmSales.length} href="/employee/crm/sales" />
    </section>

    <section className="quick-actions"><Link href="/employee/leaves">Apply for leave</Link><Link href="/employee/tasks">View tasks</Link><Link href="/employee/documents">Upload document</Link><Link href="/employee/chat">Open chat</Link><Link href="/employee/profile">View profile</Link><Link href="/employee/crm/leads">Add lead</Link><Link href="/employee/crm">Open CRM</Link><Link href="/employee/crm/follow-ups">CRM follow-ups</Link></section>

    <div className="dashboard-columns">
      <Section title="My tasks" link="/employee/tasks" linkLabel="View all tasks"><div className="dashboard-list">{displayTasks.length ? displayTasks.map((item: any) => <TaskRow key={item.id} item={item} today={today} />) : <Empty text="You have no active tasks." />}</div></Section>
      <Section title="Announcements" link="/employee/announcements" linkLabel="View all announcements"><div className="dashboard-list">{displayAnnouncements.length ? displayAnnouncements.map((item: any) => <a className="list-item" href="/employee/announcements" key={item.id}><div><b>{item.title}</b><small>{item.category} · {fmtDate(item.published_at)}</small></div><StatusBadge value={item.is_read ? 'Read' : 'New'} /></a>) : <Empty text="No announcements right now." />}</div></Section>
      <Section title="Notifications" link="/employee/notifications" linkLabel="View all notifications" action={unreadNotifications ? <button className="text-button" onClick={() => void markAllRead()}>Mark all read</button> : undefined}><div className="dashboard-list">{displayNotifications.length ? displayNotifications.map((item: any) => <a className="list-item" href={item.deep_link || '/employee/notifications'} key={item.id}><div><b>{item.title}</b><small>{item.message || item.body || 'Open to view details'} · {relativeTime(item.created_at)}</small></div><StatusBadge value={item.read_at ? 'Read' : 'New'} /></a>) : <Empty text="You are all caught up." />}</div></Section>
      <Section title="Leave requests" link="/employee/leaves" linkLabel="Manage leave"><div className="dashboard-list">{(data.leaves || []).slice(0, 3).map((item: any) => <a className="list-item" href="/employee/leaves" key={item.id}><div><b>{item.leave_types?.name || item.leave_type || 'Leave request'}</b><small>{fmtDate(item.starts_on)} – {fmtDate(item.ends_on)}</small></div><StatusBadge value={item.status} /></a>)}{!(data.leaves || []).length && <Empty text="No leave requests yet." />}</div></Section>
      <Section title="Document requests" link="/employee/documents" linkLabel="View documents"><div className="dashboard-list">{pendingDocuments.slice(0, 3).map((item: any) => <a className="list-item" href="/employee/documents" key={item.id}><div><b>{item.title || item.request_title || 'Requested document'}</b><small>Due {fmtDate(item.due_date)}</small></div><StatusBadge value={item.status} /></a>)}{!pendingDocuments.length && <Empty text="No documents are waiting for you." />}</div></Section>
      <Section title="Upcoming" link="/employee/tasks" linkLabel="View tasks"><div className="dashboard-list">{upcoming.length ? upcoming.map((item, index) => <div className="list-item" key={`${item.kind}-${index}`}><div><b>{item.label}</b><small>{item.kind} · {fmtDate(item.date)}</small></div><StatusBadge value={item.kind} /></div>) : <Empty text="Nothing upcoming at the moment." />}</div></Section>
    </div>
  </div>;
}

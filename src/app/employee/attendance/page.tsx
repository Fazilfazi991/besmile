...
export default function AttendancePage() {
  ...
  return <section className="space-y-4">
    ...
    <div className="flex flex-wrap gap-2">{['present', 'late', 'absent', 'leave', 'holiday', 'weekend'].map(status => <span className="flex items-center gap-1.5 rounded-full border border-slate-200 bg-white px-2.5 py-1 text-xs text-slate-600" key={status}><span className={`h-2 w-2 rounded-full ${status === 'present' ? 'bg-emerald-500' : status === 'late' ? 'bg-amber-500' : status === 'absent' ? 'bg-rose-500' : status === 'leave' ? 'bg-sky-500' : status === 'holiday' ? 'bg-violet-500' : 'bg-slate-400'}`} />{label(status)}</span>)}</div>
    <EmployeeSection title="Monthly calendar" description="Select a day to see attendance details."><div className="mx-auto grid max-w-6xl grid-cols-7 gap-1.5 p-3 sm:gap-2 md:p-4">{weekdayNames.map(name => <div className="pb-1 text-center text-[10px] font-bold uppercase tracking-wide text-slate-500 sm:text-xs" key={name}>{name}</div>)}{Array.from({ length: firstOffset }).map((_, index) => <div aria-hidden="true" key={`empty-${index}`} />)}{data.days.map((day: any) => { const isToday = day.key === today; const selectedDay = selected?.key === day.key; const worked = minutes(day.row); const statusText = day.status === 'future' ? '' : label(day.status); return <button onClick={() => setSelected(day)} className={`relative min-h-[72px] rounded-lg border p-1.5 text-left transition hover:brightness-[.98] sm:min-h-[86px] sm:p-2 ${statusStyle[day.status] || statusStyle.future} ${isToday ? 'ring-2 ring-teal-500 ring-offset-1' : ''} ${selectedDay ? 'shadow-sm' : ''}`} key={day.key}><span className="block text-sm font-extrabold">{day.key.slice(-2)}</span>{isToday && <span className="mt-1 inline-flex rounded bg-teal-700 px-1.5 py-0.5 text-[9px] font-bold text-white">Today</span>}{statusText && <span className="mt-1 block text-[10px] font-semibold sm:text-xs">{statusText}</span>}{(day.status === 'present' || day.status === 'late') && <span className="mt-0.5 block text-[9px] sm:text-[10px]">{duration(worked)}</span>}</button>; })}</div></EmployeeSection>

...
import { attendanceDuration, canClockIn, classify, dateKey } from '@/lib/attendance-rules';
...
export default function AttendancePage() {
  ...
  const durationFor = (day: AttendanceDay) => attendanceDuration(day.row, { timeZone: data?.settings?.timezone || 'Asia/Kolkata', workDate: day.key, activeBreakStartedAt: day.key === today ? activeBreak?.started_at : null });
      ...
      <table aria-label="Personal attendance records">
        ...
        <tbody>
          {loading ? Array.from({ length: 7 }, (_, index) => <tr className="attendance-loading-row" key={index}><td colSpan={6}><span /></td></tr>) : visible.map(day => { const worked = durationFor(day); return <tr key={day.key}>
            ...
            <td data-label="Work Hours">{worked.minutes === null ? '—' : durationLabel(worked.minutes)}{worked.isIncomplete ? <small>Missing clock-out</small> : worked.needsReview ? <small>Requires review</small> : null}</td>

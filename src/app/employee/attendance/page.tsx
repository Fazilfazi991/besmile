...
import { attendanceDuration, canClockIn, dateKey, monthlyDays, weekday } from '@/lib/attendance-rules';
...
export default function AttendancePage() {
  ...
  const durationFor = (row: any, workDate: string, active?: any) => attendanceDuration(row, { timeZone: data?.settings?.timezone || 'Asia/Kolkata', workDate, activeBreakStartedAt: active?.started_at });
  const workingLabel = (row: any, workDate: string, active?: any) => { const result = durationFor(row, workDate, active); return result.minutes === null ? '—' : duration(result.minutes); };
  ...
  const summary = (() => {
    ...
    return {
      ...
      work: data.days.reduce((total: number, day: any) => total + (attendanceDuration(day.row, { timeZone: data.settings.timezone, workDate: day.key }).minutes || 0), 0),
    };
  })();
  ...
  return <section className="space-y-4">
    ...
    <EmployeeSection title="Today's attendance" description={activeToday?.row?.clock_in ? 'Your current attendance and working time.' : 'Start your day when you are ready.'} className="border-teal-100">
      <div className="grid gap-4 p-4 sm:p-5 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5"><Detail label="Current status" value={activeBreak ? 'On break' : todayStatus} /><Detail label="Clock in" value={time(activeToday?.row?.clock_in)} /><Detail label="Break" value={activeBreak ? `Since ${time(activeBreak.started_at)}` : activeToday?.row?.break_minutes ? `${activeToday.row.break_minutes} min` : '—'} /><Detail label="Clock out" value={time(activeToday?.row?.clock_out)} /><Detail label="Working hours" value={activeToday?.row?.clock_in ? workingLabel(activeToday.row, today, activeBreak) : '—'} /></div>

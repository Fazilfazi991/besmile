...
export default async function EmployeeLayout({ children }: { children: React.ReactNode }) {
  ...
  const { data: profile } = await serverAuthorizationRead(signal => db.from('profiles').select('full_name, role, designation, status, avatar_url').eq('id', user.id).abortSignal(signal).maybeSingle(), 'employee.profile');
  ...
  return <MobileNavigationProvider><div className="app-shell employee-shell">
    ...
    <main className="app-main">
      <header className="app-topbar"><div className="topbar-mode"><MobileNavigationTrigger /><ThemeModeSwitcher /></div><GlobalCommandCenter mode="employee" userId={user.id} /><TopbarProfileLink href="/employee/profile" name={name} subtitle={profile?.designation || 'Employee'} photoUrl={photoUrl} /></header>

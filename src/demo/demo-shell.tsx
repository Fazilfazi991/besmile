...
const demoRoutes = new Set(['/admin', '/admin/reports', '/admin/employees', '/admin/patients']);
const demoGroups: NavigationGroup[] = adminNavigation.map(group => ({ ...group, links: group.links.filter(link => demoRoutes.has(link.href)) })).filter(group => group.links.length) as NavigationGroup[];

...
const navigation = {
  admin: [
    ...
    ['My Leave', '/admin/my-leave'],
    ['Leave Approvals', '/admin/leaves'],
    ['Tasks', '/admin/tasks'],
...
const notificationRoute = (mode: Mode, item: any) => {
  ...
  if (entityId && String(item.type || '').startsWith('leave_')) return mode === 'admin' ? (['leave_approved', 'leave_rejected'].includes(String(item.type)) ? `/admin/my-leave?request=${entityId}` : `/admin/leaves?request=${entityId}`) : `/employee/leaves?request=${entityId}`;

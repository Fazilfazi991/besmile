  ...
  return (
    <section className="organization-chart-section" aria-labelledby="organization-chart-title">
      ...
      <div className="organization-chart-viewport">
        ...
        <ul className="organization-chart-mobile" aria-label="BSmile organization hierarchy">
          {roots.map((root) => <OrganizationBranch key={`mobile-${root.key}`} node={root} highlightedKey={highlightedNode?.key} profilePhoto={profilePhoto} isSelf={isSelf} />)}
        </ul>
      </div>
    </section>
  );

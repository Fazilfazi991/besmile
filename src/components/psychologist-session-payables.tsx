...
export function PsychologistSessionPayables() {
  ...
  const actionable = rows.filter(x => x.status === 'scheduled' || x.status === 'payment_due');
  ...
  const pending = actionable.reduce((n, x) => n + Number(x.payable_amount), 0);
  ...
  return <section className="space-y-5">
    ...
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5"><Metric label="Pending" value={inr(pending)} /><Metric label="Scheduled" value={scheduled.length} /><Metric label="Due soon" value={due.length - overdue.length} /><Metric label="Overdue" value={overdue.length} /><Metric label="Paid" value={paid.length} /></div>
    ...
    <div className="card overflow-x-auto"><table className="min-w-[980px] w-full text-sm"><thead><tr className="border-b"><th className="p-3 text-left">Psychologist</th><th className="p-3 text-left">Session</th><th className="p-3 text-right">Payable</th><th className="p-3 text-left">Status</th><th className="p-3 text-left">Paid</th><th className="p-3" /></tr></thead><tbody>{rows.map(x => { const isOverdue = x.status === 'payment_due' && x.due_date < today(); const awaitingSettlement = x.status === 'scheduled' || x.status === 'payment_due'; return <tr className="border-b" key={x.id}><td className="p-3">{x.clinician_name_snapshot || x.psychologist?.doctor_name}</td><td className="p-3">{x.session_date} · {x.session_duration_minutes || '—'} min<br /><small>{x.appointment_id}</small></td><td className="p-3 text-right">{inr(x.payable_amount)}</td><td className="p-3"><FinanceStatus value={isOverdue ? 'overdue' : x.status} /></td><td className="p-3">{x.paid_at ? <>{String(x.paid_at).slice(0, 10)}<br /><small>{x.paid_by_profile?.full_name || x.paid_by}</small></> : '—'}</td><td className="p-3">{canSettlePermission && awaitingSettlement && <button className="btn border" disabled={busy === x.id} onClick={() => openSettlement(x)}>Mark paid</button>}</td></tr>; })}</tbody></table>{actionable.length === 0 && <p className="border-t p-6 text-slate-500">No psychologist payments are currently awaiting settlement.</p>}</div>

...
export function PsychologistSessionPayables() {
  ...
  const [canSettlePayments, setCanSettlePayments] = useState(false);

  const load = async () => {
    if (!db) return;
    setLoading(true);
    try {
      const permissions = await grantedPermissions(db, ['psychologist_payments.view', 'psychologist_payments.manage', 'psychologist_payments.settle']);
      setAllowed(permissions.has('psychologist_payments.view'));
      setCanManage(permissions.has('psychologist_payments.manage'));
      setCanSettlePayments(permissions.has('psychologist_payments.settle'));
      ...
      const [payables, summaries, clinicianRows] = await Promise.all([q, summaryQuery, db.rpc('eligible_psychologist_payment_clinicians')]);
      ...
      if (permissions.has('psychologist_payments.settle')) {
        const accountRows = await db.rpc('eligible_psychologist_payment_accounts');
  ...
  const canSettle = canSettlePayments && settling && accountId && paidOn && !busy;

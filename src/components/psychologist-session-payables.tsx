...
export function PsychologistSessionPayables() {
  ...
  const settle = async () => {
    ...
    try {
      const { error: settlementError } = await db.rpc('settle_psychologist_session_payable', {
        target_payable: settling.id, target_account: accountId, paid_on: paidOn, method, reference: reference.trim() || null,
      });
  ...
  return <section className="space-y-5">
    ...
    {settling && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4" role="dialog" aria-modal="true" aria-labelledby="settlement-title"><div className="card w-full max-w-lg space-y-4 p-5 shadow-xl"><div><h2 id="settlement-title" className="text-xl font-bold">Confirm psychologist payment</h2><p className="text-sm text-slate-500">Review the liability and choose the account that paid it.</p></div><dl className="grid grid-cols-2 gap-3 text-sm"><div><dt className="text-slate-500">Psychologist</dt><dd className="font-medium">{settling.psychologist?.doctor_name || 'Psychologist'}</dd></div><div><dt className="text-slate-500">Amount</dt><dd className="font-medium">{inr(settling.payable_amount)}</dd></div><div><dt className="text-slate-500">Due date</dt><dd className="font-medium">{settling.due_date || 'Manual'}</dd></div></dl><label className="block text-sm font-medium">Paid from account<select className="input mt-1 w-full" aria-label="Paid from account" value={accountId} onChange={event => setAccountId(event.target.value)} required><option value="">Select an active Finance account</option>{accounts.map(account => <option key={account.id} value={account.id}>{account.name}</option>)}</select></label><label className="block text-sm font-medium">Payment date<input className="input mt-1 w-full" aria-label="Payment date" type="date" value={paidOn} onChange={event => setPaidOn(event.target.value)} required /></label><label className="block text-sm font-medium">Payment method<select className="input mt-1 w-full" aria-label="Payment method" value={method} onChange={event => setMethod(event.target.value)}>{['bank_transfer', 'cash', 'upi', 'card'].map(value => <option key={value} value={value}>{value.replace('_', ' ')}</option>)}</select></label><label className="block text-sm font-medium">Payment reference / note <span className="font-normal text-slate-500">(optional)</span><input className="input mt-1 w-full" aria-label="Payment reference" value={reference} onChange={event => setReference(event.target.value)} maxLength={500} /></label><div className="flex justify-end gap-2"><button className="btn border" disabled={!!busy} onClick={() => setSettling(null)}>Cancel</button><button className="btn" disabled={!canSettle} onClick={() => void settle()}>{busy ? 'Settling…' : 'Confirm payment'}</button></div></div></div>}

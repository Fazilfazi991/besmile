...
export function PatientWorkspace({ patientSlug, basePath = '/admin/patients' }: { patientSlug: string; basePath?: string }) {
  ...
  const load = async () => {
    ...
    setDocs(d.data || []);
    setActivity(e.data || []);
    const codes = ['patients.edit', 'patients.assign', 'patient_sessions.create', 'patient_notes.create', 'clinical_notes.create', 'patient_documents.upload'];
  ...
  return <section className="space-y-5">
    ...
    {tab === 'Overview' && <><div className="flex justify-end">{action('patients.edit', 'Edit Client', 'patient')}</div>{form === 'patient' && <form className="card grid gap-3 p-4 md:grid-cols-2" onChange={() => setHasUnsavedChanges(true)} onSubmit={event => save('patient', event)}>
      ...
      {perms['patients.assign'] && <label>Assigned Psychologist<select name="assigned_psychologist_id" defaultValue={p.assigned_psychologist_id || ''} className="mt-1 w-full rounded border p-2"><option value="">Unassigned</option>{staff.map(person => <option value={person.id} key={person.id}>{person.full_name}</option>)}</select></label>}

...
const snapshotTitle = 'Performance Snapshot';
const snapshotSubtitle = 'Live workload and performance overview';
...
export function TeamAttendanceStrip({ employees, loading = false, canOpenEmployees, standardHub = false }: { employees?: TeamMember[]; loading?: boolean; canOpenEmployees: boolean; standardHub?: boolean }) {
  ...
  if (!employees?.length) return <section className="team-today"><div className="team-today-heading"><div><h2>{snapshotTitle}</h2><p>{snapshotSubtitle}</p></div></div><p className="team-today-empty">No active team members to display.</p></section>;
  ...
  return <section className={`team-today${standardHub ? ' standard-team-today' : ''}`} aria-labelledby="performance-snapshot-title"><div className="team-today-heading"><div><h2 id="performance-snapshot-title">{snapshotTitle}</h2><p>{snapshotSubtitle}</p></div>{canOpenEmployees && <Link href="/admin/attendance" className="team-today-link">View attendance</Link>}</div><div className="team-today-scroll" aria-label="Active employee performance"><div className="team-today-list">{uniqueEmployees.map((employee, index) => <TeamAttendanceCard employee={employee} canOpenEmployees={canOpenEmployees} duplicatePhoto={photoUseCount.get(employee.photo_url || '')! > 1} standardHub={standardHub} accent={index % 5} key={employee.id} />)}</div></div></section>;
}
...
function TeamAttendanceCard({ employee, canOpenEmployees, duplicatePhoto, standardHub, accent }: { employee: TeamMember; canOpenEmployees: boolean; duplicatePhoto: boolean; standardHub: boolean; accent: number }) {
  ...
  return canOpenEmployees ? <Link href={`/admin/employees/${employee.id}`} prefetch={false} className={`team-member-card${standardHub ? ` standard-team-card accent-${accent}` : ''}`}>{content}</Link> : <article className="team-member-card">{content}</article>;
}
...
function TeamAttendanceSkeleton() { return <section className="team-today" aria-label="Loading performance snapshot"><div className="team-today-heading"><div><h2>{snapshotTitle}</h2><p>{snapshotSubtitle}</p></div></div><div className="team-today-scroll"><div className="team-today-list">{Array.from({ length: 5 }).map((_, index) => <div className="team-member-card team-member-skeleton" key={index}><span className="team-member-avatar" /><div><i /><i /></div><i /></div>)}</div></div></section>; }

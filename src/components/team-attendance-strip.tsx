...
export function TeamAttendanceStrip({ employees, loading = false, error = '', onRetry, canOpenEmployees, standardHub = false }: { employees?: TeamMember[]; loading?: boolean; error?: string; onRetry?: () => void; canOpenEmployees: boolean; standardHub?: boolean }) {
  if (loading) return <TeamAttendanceSkeleton />;
  if (error) return <section className="team-today"><div className="team-today-heading"><div><h2>Performance Snapshot</h2><p>Live workload and performance overview</p></div></div><p className="team-today-empty">Performance Snapshot could not be loaded. {onRetry && <button className="ml-2 font-semibold text-teal-700 underline" onClick={onRetry}>Retry</button>}</p></section>;
  if (!employees?.length) return <section className="team-today"><div className="team-today-heading"><div><h2>Performance Snapshot</h2><p>Live workload and performance overview</p></div></div><p className="team-today-empty">No active team members to display.</p></section>;
  ...
  return <section className={`team-today${standardHub ? ' standard-team-today' : ''}`} aria-labelledby="performance-snapshot-title"><div className="team-today-heading"><div><h2 id="performance-snapshot-title">Performance Snapshot</h2><p>Live workload and performance overview</p></div>{canOpenEmployees && <Link href="/admin/attendance" className="team-today-link">View attendance</Link>}</div><div className="team-today-scroll" aria-label="Active employee performance"><div className="team-today-list">{uniqueEmployees.map((employee, index) => <TeamAttendanceCard employee={employee} canOpenEmployees={canOpenEmployees} duplicatePhoto={photoUseCount.get(employee.photo_url || '')! > 1} standardHub={standardHub} accent={index % 5} key={employee.id} />)}</div></div></section>;
}
...
function TeamAttendanceSkeleton() { return <section className="team-today" aria-label="Loading performance snapshot"><div className="team-today-heading"><div><h2>Performance Snapshot</h2><p>Live workload and performance overview</p></div></div><div className="team-today-scroll"><div className="team-today-list">{Array.from({ length: 5 }).map((_, index) => <div className="team-member-card team-member-skeleton" key={index}><span className="team-member-avatar" /><div><i /><i /></div><i /></div>)}</div></div></section>; }

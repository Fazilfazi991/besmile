...
export function TeamAttendanceStrip({ employees, loading = false, canOpenEmployees, standardHub = false, attendanceAvailable = true, leaveAvailable = true }: { employees?: TeamMember[]; loading?: boolean; canOpenEmployees: boolean; standardHub?: boolean; attendanceAvailable?: boolean; leaveAvailable?: boolean }) {
  ...
  return <section className={`team-today${standardHub ? ' standard-team-today' : ''}`} aria-labelledby="team-today-title"><div className="team-today-heading"><div><h2 id="team-today-title">Team Today</h2><p>Live attendance overview</p></div>{canOpenEmployees && <Link href="/admin/attendance" className="team-today-link">View attendance</Link>}</div>{(!attendanceAvailable || !leaveAvailable) && <p className="team-today-empty" role="status">{!attendanceAvailable ? 'Attendance status is temporarily unavailable.' : 'Leave status is temporarily unavailable.'}</p>}<div className="team-today-scroll" aria-label="Active employee attendance"><div className="team-today-list">{uniqueEmployees.map((employee, index) => <TeamAttendanceCard employee={employee} canOpenEmployees={canOpenEmployees} duplicatePhoto={photoUseCount.get(employee.photo_url || '')! > 1} standardHub={standardHub} accent={index % 5} attendanceAvailable={attendanceAvailable} leaveAvailable={leaveAvailable} key={employee.id} />)}</div></div></section>;
}

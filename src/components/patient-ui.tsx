...
export function PatientForm() {
  ...
  return <section className="max-w-3xl space-y-5"><div><h1 className="text-2xl font-bold">Add Client</h1><p className="text-slate-600">Only enter information needed for care administration.</p></div>{notice && <p role="status" aria-live="polite" className="rounded border border-emerald-200 bg-emerald-50 p-3 text-emerald-800">{notice}</p>}{error && <p role="alert" className="text-rose-700">{error}</p>}<form onSubmit={submit} className="card grid gap-4 p-5 md:grid-cols-2">{[['patient_number', 'Client ID'], ['full_name', 'Full name'], ['phone', 'Phone'], ['email', 'Email'], ['date_of_birth', 'Date of birth'], ['nationality', 'Nationality'], ['preferred_language', 'Preferred language'], ['emergency_contact_name', 'Emergency contact name'], ['emergency_contact_relationship', 'Relationship'], ['emergency_contact_phone', 'Emergency phone']].map(([name, label]) => <label key={name}>{label}<input required={name === 'patient_number' || name === 'full_name'} name={name} type={name === 'date_of_birth' ? 'date' : 'text'} className="mt-1 w-full rounded border p-2" /></label>)}<label>Gender<select name="gender" required defaultValue="" className="mt-1 w-full rounded border p-2"><option value="" disabled>Select gender</option>{genderOptions.map(option => <option value={option.value} key={option.value}>{option.label}</option>)}</select></label><label>Source<select name="source" required defaultValue="" className="mt-1 w-full rounded border p-2"><option value="" disabled>Select source</option>{patientSourceOptions.map(option => <option value={option.value} key={option.value}>{option.label}</option>)}</select></label><label>Status<select name="status" className="mt-1 w-full rounded border p-2"><option value="active">Active</option><option value="inactive">Inactive</option><option value="discharged">Discharged</option></select></label><label>Assigned Psychologist<select name="assigned_psychologist_id" className="mt-1 w-full rounded border p-2"><option value="">Unassigned</option>{staff.map(person => <option value={person.id} key={person.id}>{person.full_name}</option>)}</select></label><label className="md:col-span-2">Address<textarea name="address" className="mt-1 w-full rounded border p-2" /></label><button className="w-fit rounded bg-slate-900 px-4 py-2 text-white disabled:opacity-60" disabled={saving}>{saving ? 'Creating...' : 'Create Client'}</button></form></section>;
}
...
export function PatientProfile({ patientId }: { patientId: string }) {
  ...
  const load = async () => {
    ...
    if (p.error) setError('Client is unavailable or you do not have access.');

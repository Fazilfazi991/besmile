...
export function ChatHub() {
  ...
  const load = async (selectedId?: string) => {
    try {
      ...
      setActive(next || null);
      setError('');
    } catch (cause: any) { setError(cause.message || 'Chat could not be loaded.'); }
  ...
  const send = async (event?: FormEvent) => {
    ...
    try { await employeeRepository.sendMessage({ conversation_id: active.conversation_id, sender_id: profile.id, body: pending.body, file }); await load(active.conversation_id); }
  ...
  const create = async (person?: any) => {
    try {
      ...
      setNewChat(false); setGroup({ title: '', description: '', type: 'general', members: [] }); await load(id);
  ...
  return <section className={`chat-hub ${active ? 'chat-open' : ''}`}>
    ...
    {newChat && <div className="chat-modal-backdrop"><section className="chat-modal"><header><div><h2>{mode === 'direct' ? 'Start a direct message' : 'Create a group'}</h2><p>Only active employees can be added.</p></div><button onClick={() => setNewChat(false)}>Close</button></header>{mode === 'group' && <div className="chat-group-fields"><input className="input" placeholder="Group name" value={group.title} onChange={event => setGroup({ ...group, title: event.target.value })} /><input className="input" placeholder="Description (optional)" value={group.description} onChange={event => setGroup({ ...group, description: event.target.value })} /><select className="input" value={group.type} onChange={event => setGroup({ ...group, type: event.target.value })}><option value="general">General</option><option value="department">Department</option><option value="team">Team</option><option value="management">Management</option><option value="project">Project</option></select></div>}<input className="input" placeholder="Search employees" value={peopleQuery} onChange={event => setPeopleQuery(event.target.value)} /><div className="chat-people-list">{people.map(person => <button key={person.id} className={group.members.includes(person.id) ? 'selected' : ''} onClick={() => mode === 'direct' ? void create(person) : setGroup(current => ({ ...current, members: current.members.includes(person.id) ? current.members.filter(id => id !== person.id) : [...current.members, person.id] }))}><Avatar name={person.full_name} /><span><b>{person.full_name}</b><small>{person.designation || 'Employee'} - {person.department?.name || 'No department'}</small></span>{mode === 'group' && <i>{group.members.includes(person.id) ? 'Selected' : 'Add'}</i>}</button>)}</div>{mode === 'direct' ? <button className="chat-mode-switch" onClick={() => setMode('group')}>Create a group instead</button> : <footer><button className="button button-secondary" onClick={() => setMode('direct')}>Direct message</button><button className="btn btn-primary" disabled={!group.title || !group.members.length} onClick={() => void create()}>Create group</button></footer>}</section></div>}
  </section>;
}

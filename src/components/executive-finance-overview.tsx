...
export function ExecutiveFinanceOverview({ transactions, range }: Props) {
  ...
  return <section className={styles.overview} aria-labelledby="executive-finance-title">
    ...
    <div className={styles.grid}>
      ...
      <section className={`${styles.panel} ${styles.breakdownPanel}`} aria-labelledby="expense-breakdown-title"><h3 id="expense-breakdown-title">Expense Breakdown</h3>{view.breakdown.length ? <div className={styles.breakdown}><div className={styles.donut} role="img" aria-label={view.breakdown.map(row => `${row.name}: ${chartInr(row.value)}, ${row.percent.toFixed(0)} percent`).join('; ')}><div className={styles.donutPlot}><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={view.breakdown} dataKey="value" nameKey="name" innerRadius="61%" outerRadius="88%" startAngle={90} endAngle={-270} paddingAngle={1} stroke="none" isAnimationActive={false} onMouseEnter={(_, index) => setActiveExpense(index)} onMouseLeave={() => { if (window.matchMedia('(hover: hover)').matches) setActiveExpense(null); }} onTouchStart={(_, index) => setActiveExpense(index)} onClick={(_, index) => setActiveExpense(index)}>{view.breakdown.map((row, index) => <Cell key={row.name} fill={colors[index % colors.length]} />)}</Pie></PieChart></ResponsiveContainer><div className={styles.donutCenter}><small>Total Expenses</small><strong>{compactInr(view.expenses)}</strong></div></div><div className={`${styles.donutDetail}${activeBreakdown ? ` ${styles.donutDetailActive}` : ''}`} role="status" aria-live="polite">{activeBreakdown ? <><strong>{activeBreakdown.name}</strong><span>{chartInr(activeBreakdown.value)} · {activeBreakdown.percent.toFixed(0)}%</span></> : <span>Hover or tap a slice for details</span>}</div></div><div className={styles.breakdownList}>{breakdownLegend.map(({ row, index }) => <div className={styles.breakdownRow} key={row.name}><i style={{ background: colors[index % colors.length] }} /><span>{row.name}</span><small><span>{compactInr(row.value)}</span><span aria-hidden="true">·</span><span>{row.percent.toFixed(0)}%</span></small></div>)}</div></div> : <Empty text="No expense activity in this period" />}</section>

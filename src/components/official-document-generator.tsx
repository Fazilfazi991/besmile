...
export default function OfficialDocumentGeneratorPage() {
  ...
  const availableTypes = useMemo(() => officialDocumentTypes.filter((type) => context?.allowedTypes.includes(type.key)), [context]);
  const config = useMemo(() => officialDocumentTypes.find((item) => item.key === form.documentType)!, [form.documentType]);
  const update = (key: keyof OfficialDocumentInput, value: string) => setForm((current) => {
    const next = { ...current, [key]: value };
    if (
      current.documentType === 'offer_letter'
      && ['relatedName', 'position', 'joiningDate'].includes(key)
      && current.body === templateBody('offer_letter', current.relatedName, current.position, current.joiningDate)
    ) next.body = templateBody('offer_letter', next.relatedName, next.position, next.joiningDate);
    return next;
  });

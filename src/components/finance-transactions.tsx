...
export function FinanceTransactions({ type }: { type: "income" | "expense" }) {
  ...
  const [archiveTarget, setArchiveTarget] = useState<any>();
  const [archiveBusy, setArchiveBusy] = useState(false);
  const [archiveError, setArchiveError] = useState("");
  const categories =
    type === "income"
      ? options.income
      : options.expense.filter((category: any) =>
          isCanonicalExpenseCategory(category.name),
        );
  const load = async () => {
    try {
      setLoading(true);
  ...
  const save = async (event: FormEvent) => {
    ...
    try {
      ...
      if (
        type === "expense" &&
        !isCanonicalExpenseCategory(
          categories.find((category: any) => category.id === edit[categoryKey])
            ?.name,
        )
      )
        throw new Error(
          "Select Capital Expense, Monthly Expense, or Maintenance.",
        );
  ...
  const archive = async (row?: any) => {
    ...
    try {
      if (archiveTarget.archived_at)
        await adminRepository.restoreFinanceTransaction(archiveTarget.id);
      else await adminRepository.archiveFinanceTransaction(archiveTarget.id);
      setNotice({
        type: "success",
        message: `Transaction ${archiveTarget.archived_at ? "restored" : "archived"}.`,
      });
  ...
  const categoryGuidance =
    type === "expense"
      ? canonicalExpenseCategoryNames
          .map((name) => `${name} — ${expenseCategoryHelp[name]}`)
          .join(" ")
      : "";
  if (archiveTarget)
    return (
      <ConfirmationDialog
        ...
        description={`${archiveTarget.description || archiveTarget.counterparty_name || "This transaction"} will ${archiveTarget.archived_at ? "be restored" : "be archived"}.`}
        confirmLabel={archiveTarget.archived_at ? "Restore" : "Archive"}
        destructive={!archiveTarget.archived_at}
        pending={archiveBusy}
        error={archiveError}
        onClose={() => {
          if (!archiveBusy) {
          ...
          >
            ...
            <div className="mt-5 grid gap-4 md:grid-cols-2">
              ...
              <Field label="Category">
                ...
                {type === "expense" && (
                  <small className="mt-1.5 block text-xs font-normal leading-5 text-slate-500">
                    {categoryGuidance}
                  </small>
                )}
              </Field>

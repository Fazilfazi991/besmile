...
Missing credentials, a database probe failure, or any critical browser failure produces `RELEASE GATE FAIL`. Production smoke is separate: set the production base URL and dedicated smoke credentials, explicitly set `BSMILE_PRODUCTION_SMOKE=1`, then run `pnpm run smoke:production`. It performs navigation-only checks and must never use client records.
